...
def test_build_whisper_prompt_contains_top_brands():
    """Содержит ключевые AI-инструменты которые Максим упоминает в роликах."""
    result = build_whisper_prompt()
    must_have = ["ChatGPT", "Claude", "Gemini", "Cursor", "Midjourney"]
    missing = [b for b in must_have if b not in result]
    assert not missing, f"Missing AI tools in prompt: {missing}"
    print(f"  OK contains all top AI tools: {must_have}")


def test_build_whisper_prompt_contains_maksim_brand():
    """Содержит бренд и бизнес-термины Максима — то, что чаще всего звучит."""
    result = build_whisper_prompt()
    must_have = ["Life Drive", "Юмсунов", "картинг", "глэмпинг", "Тюмень"]
    missing = [b for b in must_have if b not in result]
    assert not missing, f"Missing Maksim brand/business terms: {missing}"
    print(f"  OK contains Maksim brand terms: {must_have}")
