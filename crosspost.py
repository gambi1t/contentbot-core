...
YOUTUBE_REDIRECT_URI = "https://bot.panferov-ai.ru/oauth/callback"
YOUTUBE_TOKEN_FILE = Path(__file__).parent / "youtube_token.json"

# Meta / Instagram (auth goes through Facebook OAuth)
META_APP_ID = os.getenv("META_APP_ID", "921654167162123")
META_APP_SECRET = os.getenv("META_APP_SECRET", "4919185465ac427d4553b7511d1fc61e")
...
async def telegram_post_to_channel(bot, text: str, video_path: str | None = None) -> dict | None:
    """
    Post to Telegram channel.
    - If video_path is provided, sends video with caption.
...
def upload_video_to_temp_hosting(video_path: str) -> str | None:
    ...
    Copies to /root/content-bot/media/ served by nginx at bot.panferov-ai.ru/media/
    ...
    try:
        ...
        dest.chmod(0o644)
        url = f"https://bot.panferov-ai.ru/media/{filename}"
...
def tiktok_upload_video(
    ...
    # interpreter as the bot (main venv at /root/content-bot/venv/bin/python).
...
"""
    ...
    try:
        result = subprocess.run(
            ...
            cwd="/root/content-bot",  # cookies are here: TK_cookies_{account}.json
...
VK_REDIRECT_URI = "https://bot.panferov-ai.ru/oauth/vk/callback"
