...
def _tenant_theme_brief() -> str:
    """C (per-tenant тема): мир/метафоры/запретные образы бренда для промптов HF.

    Стирает «трассу»/картинг у panferov (раньше единственная тема = Life Drive).
    ...
    if tid == "panferov":
        return (
            "🎯 ТЕМА БРЕНДА (panferov / «Илон» — AI-контент-завод):\n"
            "- Мир: AI-инструменты, автоматизация контента, пайплайны, агенты, "
            "нейросети, потоки данных, дашборды, продуктивность.\n"
            "- Метафоры: пайплайн (ноды, соединённые сигнал-линиями), нейро-граф, "
            "поток данных сквозь кадр, дашборд с метриками, шестерёнки-автоматизация, "
            "множитель ×N, было→стало (руками→ИИ).\n"
            "- Архетип path_map = ДАТА-ПАЙПЛАЙН (ноды вход→агент→агент→выход, связи "
            "рисуются линиями, частица-данные течёт по узлам), а НЕ извилистая "
            "дорога/трасса с точкой.\n"
            "- 🔴 ЗАПРЕЩЁННЫЕ ОБРАЗЫ (НЕ рисовать и не упоминать): автодороги/трассы/"
            "гонки/руль/машины/карты(go-kart), палатки/кемпинг/природа/сезоны/времена "
            "года, рубли-выручка-резервы, картинг, глэмпинг, Тюмень, Life Drive — "
            "это ЧУЖОЙ бренд.\n"
            "- Цифры иллюстративные: «×40 постов/мес», «92% на автомате», «8ч руками "
            "→ 30мин с ИИ». НЕ финансовая отчётность в рублях.\n"
            "- Визуальные стили под тему: data_drift (ядро — AI/ML), swiss_pulse "
            "(метрики/дашборды), shadow_cut (драма до/после ИИ), maximalist_type (CTA)."
        )
    return (
        "🎯 ТЕМА БРЕНДА (Life Drive — картинг + глэмпинг, Тюмень):\n"
        "- Мир: активный отдых, картинг, глэмпинг, сезонный бизнес предпринимателя.\n"
        "- Цифры бизнеса иллюстративные (проценты/месяцы простоя), НЕ выдуманные "
        "точные рубли выручки."
    )
