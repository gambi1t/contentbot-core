...
def _tenant_theme_brief() -> str:
    """C (per-tenant КОНТЕКСТ канала): домен + аудитория для промптов HF, чтобы
    генерация шла ОТ СЦЕНАРИЯ и была РАЗНООБРАЗНОЙ, а не копировала тематику
    чужого бренда по инерции.

    БЕЗ чёрного списка образов (Артём 23.06): запрещаем не дороги/природу/деньги,
    а авто-подстановку Максим-специфики (Life Drive/картинг). Если фрагмент сам
    требует образа — он допустим, когда реально иллюстрирует смысл.
    tenant.active_tenant_id() (lazy import — модуль остаётся импортируемым)."""
    try:
        import tenant
    ...
    if tid == "panferov":
        return (
            "🎯 КОНТЕКСТ КАНАЛА (panferov / «Илон»):\n"
            "- Канал об AI-инструментах, автоматизации и производстве контента; "
            "аудитория — предприниматели и практики.\n"
            "- Веди визуал ОТ СМЫСЛА КАЖДОГО ФРАГМЕНТА сценария и делай его "
            "РАЗНООБРАЗНЫМ — задействуй весь арсенал (пайплайны, ноды, потоки данных, "
            "дашборды, метрики, ×N, нейро-графы — и любые ДРУГИЕ уместные образы по "
            "смыслу). Не лепи один и тот же шаблон во всех сценах.\n"
            "- Это НЕ канал про картинг/глэмпинг/сезонный бизнес — НЕ подставляй "
            "тематику Life Drive по инерции (она была заточена под другого клиента). "
            "Но и не ограничивай себя: если фрагмент сам требует дороги, природы, "
            "денег — образ допустим, когда он реально иллюстрирует смысл.\n"
            "- Цифры иллюстративные под смысл (×N постов, % автоматизации, часы), "
            "без выдуманной точной финансовой отчётности."
        )
    return (
        "🎯 КОНТЕКСТ КАНАЛА (Life Drive — картинг + глэмпинг, Тюмень):\n"
        "- Канал об активном отдыхе и сезонном бизнесе предпринимателя.\n"
        "- Веди визуал от смысла фрагмента и делай его разнообразным. Цифры "
        "иллюстративные (проценты/месяцы), без выдуманных точных рублей выручки."
    )
...
- Слова НЕ рвутся посреди: запрещены word-break:break-all и overflow-wrap:anywhere.
  Латиница (любое латинское название) и любое слово переносятся ЦЕЛИКОМ. Если делаешь
...
def _build_prompt(script_text: str, fix_error: str | None = None,
    ...
    return f"""Ты — моушн-дизайнер студии. Создай {N_INSERTS} коротких графических
B-roll-вставок под сценарий ролика для Telegram-канала, используя HyperFrames.
Это АВТОНОМНЫЙ режим: не задавай вопросов, не используй AskUserQuestion.

{_tenant_theme_brief()}

СЦЕНАРИЙ (озвучка аватара):
─────────────────────────────────────
{script_text}
─────────────────────────────────────
{storyboard_block}
...
ЧТО СДЕЛАТЬ:
- Раздели сценарий на {N_INSERTS} визуальных моментов в хронологическом порядке.
- Создай {N_INSERTS} ОТДЕЛЬНЫХ STANDALONE-композиций в КОРНЕ проекта:
  `scene_01.html` … `scene_{N_INSERTS:02d}.html`. scene_01 — первый момент,
  scene_{N_INSERTS:02d} — последний.
...
- Стиль — по активному style-контракту тенанта (style_contract.<tenant>.json):
  фон тёмный, акцентный цвет берётся ИЗ контракта (НЕ хардкодить — иначе протечка
  бренда между тенантами; страж tests/test_core_no_brand_leak.py).
